...
?>
<body style="width:full;height:full;background-color:rgb(255,247,236); background-size:cover;">
<div style="margin-up : 100px">
<div class="container">
    <div class="mb-3">
<?php
foreach($users as $user){
$orders=mysqli_query($con,"SELECT * FROM `history` WHERE `history`.`User`='$user' ORDER BY `history`.`Date`");
$date=array();
$cnt=1;
...
<table >
    ...
    Owner is : <?= $carddetails["Owner"] ?>
        ...
        <form method="post">
            ...
            <input type="submit" name="send" class="btn btn-success" value="Change" />
        </form>
        <?php
        }else{
        ?>
...
if(isset($_POST["send"])){
    $user=$_POST["User"];
    $date=$_POST["date"];
    mysqli_query($con,"UPDATE `history` SET `Status`='Checked By Manager' WHERE `history`.`User`='$user' AND `history`.`Date`='$date'");
    header("refresh:0.4;url=ordersM.php");
    ob_end_flush();
}
